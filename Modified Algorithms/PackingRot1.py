...
def widthFind(width, rot, rot2):
  ...
  return max(a, b)
def pixArr(widths, side):
  images = {}
  for i in range(len(widths)-1):
    images[i] = []
    u = Image.open(str(i+1)+".png")
    widthh, heightt = u.size
    for j in range(27):
      v = u.rotate(j*360.0/27.0)
      w, h = v.size
      images[i].append(truncate(pixelate(str(i+1)+".png", u, j*360.0/27.0, w/widthh*widths[i], side, i+2)))
  return images
def size(x):
  count = 0
  for i in range(len(x)):
...
def imageCreator(arr, scale):
  ...
  out.save("final.jpg")
def pack(widths, side):
  images = pixArr(widths, side)
...
def rotPlace(container, bumperContainer, img, bumpImg, side):
  ...
  for j in range(3):
    c1 = (current + 3**(2-j)) % 27
    c2 = (current - 3**(2-j)) % 27
    temp1, temp2 = place(bumperContainer, bumpImg[c1])
    if temp1 < minW:
      print(":)")
      minH, minW = temp1, temp2
      current = c1
    else:
      temp1, temp2 = place(bumperContainer, bumpImg[c2])
      if temp1 < minW:
          minH, minW = temp1, temp2
          current = c2
  placeHelp(container, img[current], minW, minH)
  return (current, minH, minW)
...
widths = [2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,2, 2, 2, 2, 2, 2, 2, 2,  2, 2,10]
...
duplicate("images.png", 101)
imageCreator(pack(widths, 0.04), 3)